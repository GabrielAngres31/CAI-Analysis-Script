%web_drop_table(WORK.PARL0);

FILENAME REFFILE0 '/folders/myfolders/PARL0.csv';
...
PROC CONTENTS DATA=WORK.PARL0; RUN;

* %web_open_table(WORK.PARL0);
* %web_open_table(WORK.PARL1);

DATA WORK.OutStatesP0;
...
proc sort data=WORK.PARL0 out=Work.TempDataSorted;
	by Accession;
run;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_W;
RUN;
DATA WORK.OutInterceptsP0;
	SET WORK.OutInterceptsP0 _selsum_W;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_WH;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_WD;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_WT;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_HD;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_HT;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_DT;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_WHD;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_WHT;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_WDT;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_HDT;
RUN;
...
RUN; 
DATA WORK.OutStatesP0;
	SET WORK.OutStatesP0 _fstat_WHDT;
RUN;
...
PROC EXPORT DATA = WORK.OUTSTATESP0
	REPLACE
	DBMS = CSV
	OUTFILE = '/folders/myfolders/PARL0_STATS.csv';
RUN;

PROC EXPORT DATA = WORK.OUTINTERCEPTSP0
	REPLACE
	DBMS = CSV
	OUTFILE = '/folders/myfolders/PARL0_STAT-INTS.csv';
RUN;
