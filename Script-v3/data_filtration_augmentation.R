...
library("data.table")
>>>>>>> parent of 97c7994 (Finished out main functionality and streamlined file generation)
...
UTILITY.datapointFlagger <- function(max_flags = 1, leverage_constant = 3, source_data = DATA.unfiltered, file_models_for_filtering = "leverage_filter_models.txt") {
  ...
  for (model in models_for_filtering) {
    for (acc in accessions_possible) {
      leverage_decision <- GENERATOR.modelFinder(model, acc, return_lm_object = TRUE) %>%
...
GENERATOR.modelFinder <- function(model_string, accession_choice = "All", return_params = c("All"), source_data = DATA.unfiltered, return_lm_object = FALSE) {
...
GENERATOR.heatmap_table <- function(models_and_abbreviations_df, model_statistic, source_data = DATA.unfiltered) {
  
  # Verify that the input models dataframe only contains the model and abbreviation columsn that we need
...
GENERATOR.fitline_plot <- function(model_string, accession_choice, source_data = DATA.unfiltered, source_data_name = "Unfiltered Data") {
  ...
  model_to_plot <- GENERATOR.modelFinder(model_string, accession_choice, return_params = c("Adj. R^2", "P-value", "Coefficients", "Model"), source_data)
...
for (measure in DATA.initial_measures) {
  ...
  #   Hypergraph
  UTILITY.quickPNG((paste0("Hypergraph-", measure, "--UNF")), "image_output\\tukeyplots\\hypergraphs")
  ...
  #   Hypergraph
  UTILITY.quickPNG((paste0("Hypergraph-", measure, "--FIL")), "image_output\\tukeyplots\\hypergraphs")
